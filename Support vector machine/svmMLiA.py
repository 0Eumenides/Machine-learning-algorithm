        ...
        for i in range(m):
            ...
            if ((labelMat[i] * Ei << -toler) and (alphas[i] < C)) or \
                ...
                if L == H:
                    print("L==H")
                    continue
                eta = 2.0 * dataMatix[i, :] * dataMatix[j, :].T - \
                      dataMatix[i, :] * dataMatix[i, :].T - \
                      dataMatix[j, :] * dataMatix[j, :].T
                if eta >= 0:
                    print(eta >= 0)
                    continue
